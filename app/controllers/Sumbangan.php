...
class Sumbangan extends \Core\Controller
{
    ...
    {   
        ...
        foreach($jumlah as $jm){
            if($jm == "" || $jm == 0 ) $note = "Silahkan cantumkan barang yang ingin didonasikan";
        }
        foreach($barang as $brg){
            if($brg == "") $note = "Sertakan jenis barang yang didonasikan";
        }
        if($_POST['name'] == "") $note = "Silahkan isi nama anda";
        if($note != ""){
            $result = sumbang::getName();
            View::renderTemplate("sumbangan/index.html", [
        ...
        View::renderTemplate("sumbangan/index.html", [
            'judul' => 'Donasi COVID-19',
            'jenis' => $result,
            'note' => 'Terima kasih atas donasi yang telah diberikan, semoga bermanfaat bagi yang membutuhkan',
        ]);
